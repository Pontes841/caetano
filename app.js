...
// Função para atualizar o statusco no banco de dados (controle de cobrança)
...
// Função para atualizar o statusvd no banco de dados (controle OS)
...
// Função para atualizar o statusfn no banco de dados finalização
...
// Função para atualizar o statusad no banco de dados Adaptação (controle OS)
...
// Função para atualizar o statusip no banco de dados importancia de cuidar do seus oculos (controle OS)
...
// Função para atualizar o sattusde no banco de dados Desconto (controle OS)
...
// Função para atualizar o statuscol no banco de dados coleção nova (controle OS)
...
// Função para atualizar o statusan no banco de dados aniversario (controle de OS)
...
// Função para atualizar o statusip2 importancia de cuidar dos seus oculos (controle de OS) 2
...
// Função para atualizar o statusip3 importancia de cuidar dos seus oculos (controle de OS) 3
...
// Função para atualizar o statusiip4 importancia de cuidar dos seus oculos (controle de OS) 4
...
// Função para atualizar o statusiip5 importancia de cuidar dos seus oculos (controle de OS) 5
...
// Função para atualizar o statusiip5 importancia de cuidar dos seus oculos (controle de OS) 6
...
// Função para atualizar o renovação dos oculos (controle de OS)
...
// Função para obter os registros de agendamento do banco de dados statusad (controle OS)
...
// Função para obter os registros de agendamento do banco de dados statusip importancia de cuidar dos seus oculos (controle OS)
...
// Função para obter os registros de agendamento do banco de dados desconto (controle OS)
...
// Função para obter os registros de agendamento do banco de dados coleçãp nova (controle OS)
...
// Renovação dos oculos (controle OS)
...
io.on('connection', function (socket) {
    ...
    // Função para verificar se o tempo mínimo de espera entre mensagens foi atingido
    function verificarTempoEspera() {
        const agora = new Date();
        if (ultimoEnvio === null || (agora - ultimoEnvio) / 1000 >= 60) {
            // Tempo mínimo de espera atingido, redefinir contador e último envio
            contadorMensagens = 0;
            ultimoEnvio = agora;
            return true;
        } else {
    ...
    // mensagem desconto (controle OS)
            ...
            // Mensagem coleção nova (controle OS)
            ...
            for (const agendamento of agendamentosdatern) {
                if (agendamento.datern && agendamento.datern <= hoje && !agendamento.enviado) {
                    ...
                    if (agendamento.mensagemnagemrn && agendamento.mensagemnagemrn !== '') {
                        console.log('URL da mensagemnagemrn:', agendamento.mensagemnagemrn);
                        try {
                            const media = await MessageMedia.fromUrl(agendamento.mensagemnagemrn);
                            client.sendMessage(agendamento.fone + '@c.us', media, { caption: 'Óticas Diniz' });
                        } catch (error) {
                            console.error('Erro ao obter a mensagemnagemrn:', error);
                        }
                    }
